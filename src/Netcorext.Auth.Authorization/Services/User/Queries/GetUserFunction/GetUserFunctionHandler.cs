...
public class GetUserFunctionHandler : IRequestHandler<GetUserFunction, Result<IEnumerable<Models.UserFunction>>>
{
    ...
    public GetUserFunctionHandler(DatabaseContextAdapter context, IOptions<ConfigSettings> config)
    {
    ...
    public async Task<Result<IEnumerable<Models.UserFunction>>> Handle(GetUserFunction request, CancellationToken cancellationToken = default)
    {
        if (!_config.AppSettings.Owner.IsEmpty() && _config.AppSettings.Owner.Any(t => t == request.Id))
            return Result<IEnumerable<Models.UserFunction>>.Success.Clone(new Models.UserFunction[]
                                                                          {
                                                                              new Models.UserFunction
                                                                              {
                                                                                  Functions = new Models.Function[]
                                                                                              {
                                                                                                  new Models.Function
                                                                                                  {
                                                                                                      Id = "*",
