...
public class GetUserFunctionMixRoleHandler : IRequestHandler<GetUserFunctionMixRole, Result<Models.UserFunctionMixRole>>
{
    private readonly DatabaseContext _context;
    private readonly ConfigSettings _config;

    public GetUserFunctionMixRoleHandler(DatabaseContextAdapter context, IOptions<ConfigSettings> config)
    {
        _context = context.Slave;
        _config = config.Value;
    }

    public async Task<Result<Models.UserFunctionMixRole>> Handle(GetUserFunctionMixRole request, CancellationToken cancellationToken = new())
    {
        if (!_config.AppSettings.Owner.IsEmpty() && _config.AppSettings.Owner.Any(t => t == request.Id))
            return Result<Models.UserFunctionMixRole>.Success.Clone(new Models.UserFunctionMixRole
                                                                    {
                                                                        UserFunctions = new[]
                                                                                        {
                                                                                            new Models.UserFunction
                                                                                            {
                                                                                                Functions = new[]
                                                                                                            {
                                                                                                                new Models.Function
        ...
        var emptyContent = new Models.UserFunctionMixRole
                           {
                               UserFunctions = new Models.UserFunction[request.PermissionConditions?.Length ?? 0]
                           };
        ...
        if (roleIds.Length == 0)
            return Result<Models.UserFunctionMixRole>.Success.Clone(emptyContent);
    ...
    private Task<Models.UserFunction> GetFunctionsAsync(GetUserFunctionMixRole.PermissionCondition requestPermissionCondition, IEnumerable<long> rolePermissions, IEnumerable<Models.PermissionCondition> mixingPermissionConditions, IEnumerable<Models.PermissionRule> rules)
    {
